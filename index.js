...
//   const qnswer = randoRidlle.answer;

    
  const questionElment = document.createElement('p');
  questionElment.textContent = question;
   
  const answerElment = document.createElement('p');
  answerElment.textContent ='The answer is: ' + answer;
  answerElment.setAttribute('id', 'riddle-answer');
  answerElment.hidden = true;
  ...
  container.appendChild(questionElment);
  container.appendChild(answerElment);
...
function revealAnswers() {
    const riddleContaienr = document.querySelector('.riddle-content');
    const riddle = riddleContaienr.querySelector('p');
    ...
    if (riddle && answer.hidden) {
      ...
      alert('there is no riddle reveal asnwer')
    }
